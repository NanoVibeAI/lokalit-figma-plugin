// Pure-JS SHA-256 — crypto.subtle is unavailable in Figma's sandboxed iframe
function sha256Bytes(msg: Uint8Array): Uint8Array {
  ...
  const ror = (n: number, d: number) => (n >>> d) | (n << (32 - d));
...
export function generateCodeChallenge(verifier: string): string {
  return b64url(sha256Bytes(new TextEncoder().encode(verifier)));
}
